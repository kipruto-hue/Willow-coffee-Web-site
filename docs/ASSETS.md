...
| Packaging shot, web-sized | `public/brand/packaging-amber.jpg` (1600×945), `packaging-tan.jpg` | ✅ real — the photographs above, resized only. Not traced or recoloured. **No longer used in the DOM**: the Product act now floats the cutouts below instead of this flat shot. Kept as the reference the motif was traced against. | brand-owned |
| **Packaging cutouts** | `assets-src/pouch.png`, `cup.png` → `public/media/pouch.webp` (1096×1039), `cup.webp` (717×569) | ✅ real — supplied transparent cutouts, built by `npm run build:brand`: alpha-trimmed, then WebP q88. 1.20MB of PNG → 174KB. `cup.png` arrived with 40% of its canvas as empty padding; the build trims it, or the CSS sizes a box that is mostly nothing. | brand-owned |
